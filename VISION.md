...
| 4 | `pal_sync_datasets` (+ lift the datasets-are-GUI-only rule, recreate never) | M | ✅ BUILT 0.9.0 (live dataset test pending a Quote Game unlock). Define schema in pal.json → sync provisions the table. Recreate (drop+rebuild, deletes data) gated behind an exact typed phrase — proven impossible to trigger without it (scripts/datasets-unit.js) |
