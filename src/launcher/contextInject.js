...
function syncSection(palName) {
    return [
        ...
        "",
        "- Pull the latest server state with `pal_pull` before starting significant work. Pull is a",
        "  safe sync: it preserves NEW un-pushed files (and carries their `pal.json` entries forward),",
        "  and it REFUSES — naming the files — if it would overwrite un-pushed local edits.",
        "- When you finish a meaningful change, **offer to `pal_push`** (palsync does not auto-push).",
        "- After pushing UI or workflow changes, **offer to `pal_test`** — it runs the server's real",
        "  workflow compile (catching errors `pal_push` cannot, e.g. workflow-JS that saved but won't",
        "  compile) and opens a live preview in the user's browser. Validation notes come back to you;",
        "  fix any and push again. (The preview URL carries credentials and is opened locally, not shown.)",
